...
if (!function_exists('str_contains')) {
    ...
    {
        return false !== strpos($haystack, (string) $needle);
    }
}
...
function getMarkDown($prevTag, $currTag)
{
    ...
    if ([] !== $commits) {
        ...
        foreach ($commits as $commit) {
            ...
            $commitIssuesWithUrls = array_map(static function ($issue) {
                return sprintf('[%1$s](https://github.com/horstoeko/zugferdublbridge/issues/%2$s)', $issue, substr($issue, 1));
            }, $commitIssues);
            ...
            $markDown[] = sprintf('| %6$s | [%1$s](https://github.com/horstoeko/zugferdublbridge/commit/%1$s) | %2$s | %3$s | %4$s | %5$s', $commitHash, $commitDate, $commitAuthor, $commitSubject, implode(', ', $commitIssuesWithUrls), $commitSubjectIcons);
        }
...
if (!isset($argv[1]) && !isset($argv[2])) {
    ...
    $allTags = array_filter($allTags, static function ($tag) {
        return false === str_starts_with($tag, 'v0.');
    });
